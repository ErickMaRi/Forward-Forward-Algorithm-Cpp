...
public:
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        double loss = std::log(1 + std::exp(mean_g_neg - mean_g_pos))*(mean_g_neg/mean_g_pos);
...
int main() {
    ...
    const int input_size = 64; // Size of the periodic signal in each data point
    int num_data_points = 2048; // Number of data points

    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> dist(0.85, 1.15); // Range for frequency and amplitude
    std::uniform_real_distribution<double> phase_dist(0.0, 2* M_PI); // Range for phase
    ...
    for (int i = 0; i < num_data_points; ++i) {
        ...
        for (int j = 0; j < input_size; ++j) {
            double x = j / static_cast<double>(input_size - 1); // Normalize position within the signal
            x_sawtooth[i][j] = sawtooth_amplitude * (2 * (x + sawtooth_phase - std::floor(x + sawtooth_phase + 0.5))); // Sawtooth value at position j
        }
    }
// Step 3: Initialize NeuralNetwork and add layers
    ...
    for (int i = 0; i < num_iterations; ++i) {
        ...
        // Print positive data (sine wave) for the first data point
    std::cout << "Positive data (sine wave):" << std::endl;
    for (double value : x_sine[0]) {
        std::cout << value << " ";
    }
    std::cout << std::endl;

    // Print negative data (sawtooth wave) for the first data point
    std::cout << "Negative data (sawtooth wave):" << std::endl;
    for (double value : x_sawtooth[0]) {
        std::cout << value << " ";
    }
    std::cout << std::endl;
    ...
    for (int i = 0; i < num_data_points; ++i) {
        double sine_frequency = 4*dist(gen);
        double sine_amplitude = dist(gen);
        double sine_phase = phase_dist(gen);
    ...
    for (int i = 0; i < num_data_points; ++i) {
        double sawtooth_frequency = 8*dist(gen);
        double sawtooth_amplitude = dist(gen);
        double sawtooth_phase = phase_dist(gen);

        for (int j = 0; j < input_size; ++j) {
            double x = j / static_cast<double>(input_size - 1); // Normalize position within the signal
            test_sawtooth[i][j] = sawtooth_amplitude * (2 * (x + sawtooth_phase - std::floor(x + sawtooth_phase + 0.5))); // Sawtooth value at position j
        }
    }
